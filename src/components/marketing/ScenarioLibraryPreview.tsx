...
export function ScenarioLibraryPreview() {
  const featured = FEATURED_SLUGS.map((slug) => scenarios.find((item) => item.slug === slug)).filter(
    (item): item is (typeof scenarios)[number] => Boolean(item)
  );
    ...
    <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="space-y-1">
        ...
        <p className="text-sm text-slate-700">
          Real negotiation situations designed for search intent and immediate action.
        </p>
      </div>
